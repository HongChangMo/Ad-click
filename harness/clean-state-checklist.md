...
- [ ] 이번 세션에서 완료한 기능의 `status` 가 `passing` 으로 변경됐다
- [ ] `passing` 으로 표시된 기능의 `evidence` 필드가 채워져 있다
- [ ] 실제로 검증되지 않은 기능이 `passing` 으로 표시되지 않았다
...
- [ ] 클릭 1회에 잔액이 정확히 10원만 차감된다
- [ ] 잔액이 0 미만으로 내려가지 않는다
- [ ] 잔액 소진 후 EXHAUSTED 상태 광고에 클릭 시 404가 반환된다
